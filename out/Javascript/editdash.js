...
// all the variables for assignment for all the html made variables
        var today = new Date();
  ...
  var Comment1, Comment2 , Comment3 , Comment4 , Comment5 , Comment6 , Comment7 , Comment8 , Comment9 , Comment10;
    ...
    function update()       // converts all html elements and grabs the value held in them and attributes them to a variable.
    {
...
desc4 = document.getElementById('desc4').value;
    ...
    var updatez = {     //variable that holdes what is equal to whawt ex. desc2 is desc2  html element == javascript variable
...
desc10: desc10,
    ...
    var previousData; //previous data gathering
    database.ref('Account/' + accNum).once('value', function(snapshot){
      previousData = snapshot.val();
      console.log(previousData);
      eventLog(previousData, updatez);
      database.ref('Account/'+accNum).update(updatez);//uses the update function to grab all the variables from above and update it to the specified acc Num account
    });  //adds to event log database with old data, new, and user ID
        ...
        function dater()
        {
                timeStmp(); //once time stamp is taken
            firebase.database().ref('editLog/' + id2).set({ //firebase database update the stuff
                accNum : accNum, 
                dates: dates
            });
            alert('Timestamp Made')
        }


    document.getElementById('Update').onclick = function(){     // actually calls the update function when the update button is pushed 
            ...
            } /*else ends for firebase user role */
        }); /*firebase user role collection ends*/
    };  
    ...
    function userName(){
      ...
      function eventLog(oldData, newData){
        const eventID = database.ref('Event_Log').push().key; //make event log key
        const eventLogData = { //object to push
          EventID : eventID,
          UserID : firebase.auth().currentUser.uid,
          OldData : oldData,
          NewData : newData,
          Date_Time : new Date().toLocaleDateString() //https://www.w3schools.com/jsref/jsref_tolocalestring.asp
        };
        console.log(eventLogData);
        database.ref('Event_Log').child(eventID).set(eventLogData); //throw it in firebase
      }